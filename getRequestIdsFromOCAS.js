    ...
    function processJob(job, cb) {
        ocasLogin.onLogin(function (error, authToken) {
            ...
            function createJobs(requests) {
                ...
                ddpClient.call("findRedundantJobs", [createJobConfig.root, {
                    "data.ocasRequestId": {$in: requestIds},
                    status: {$ne: "failed"}
                }, {"data.ocasRequestId": 1}], function (error, redundantJobs) {
                    if (error) {
                        job.fail({task: "findRedundantJobs", exception: error});
                        cb();
                        return;
                    }
                    var redundantIds = _.map(redundantJobs, function (job) {
                        return job.data.ocasRequestId;
                    });
                    var todoOcasRequestIds = _.difference(requestIds, redundantIds);

                    ddpClient.call("findExistingTranscripts", [{ocasRequestId: {$in: todoOcasRequestIds}}, {ocasRequestId: 1}], function (error, transcripts) {
                        if (error) {
                            job.fail({task: "findExistingTranscripts", exception: error});
                            cb();
                            return;
                        }
                        const existingTranscriptIds = _.map(transcripts, function (transcript) {
                            return transcript.ocasRequestId;
                        });
                        todoOcasRequestIds = _.difference(todoOcasRequestIds, existingTranscriptIds);
                        var jobs = todoOcasRequestIds.map(function (requestId) {
